...
- **`config/`** holds the project package.
  ...
  - **`config/log_filters.py`** (task 011) is the one definition of the rule that hides a Zoom start link's token (`/zoom/start/<token>/`) from every log the app writes — `HideStartTokenFilter`, wired onto every handler in `LOGGING`, and `docker/gunicorn.conf.py`'s access log. It lives outside `config/settings/` on purpose: gunicorn loads its config, and so this filter, *before* Django's app registry is set up, so the module must not import Django or app code. `HideStartTokenFilter.filter()` runs its redaction inside `try/except Exception` and returns `True` unchanged on any failure, so a malformed log call still reaches `logging`'s own error report instead of crashing the caller (review round 2, should-fix 1).
- **`apps/<name>/`** holds the local apps.
  ...
  - **Current apps:**
    ...
    - `zoom`: Zoom link requests, task 005; live Zoom connection, task 006; cancel a booking and a start link instead of the emailed host key, task 011. A public request → a verified email → the IT queue → conflict-free host assignment → the link email. `providers.py` is a swappable interface: `fake` for dev/tests, `manual` as a production fallback that can't see meetings made directly in Zoom (it raises the deploy warning `zoom.W001`), and `zoom`, the live provider, for production once every paid account is connected. `apps/zoom/zoom_api.py` is the only module that talks HTTP to Zoom — the only importer of `requests`, holding the fixed hosts, the Server-to-Server OAuth token cache, timeouts and the error mapping. Host keys are stored Fernet-encrypted at rest, never in plaintext; Zoom credentials never touch the database at all (env only, task 006 D2). `services.approve()` retries once on a MySQL deadlock, so call it only outside an open `transaction.atomic()` block — nesting one breaks the retry's savepoint (review round 2, nit 1). With the `zoom` provider, `approve()` also asks Zoom whether the chosen account is free *before* taking any locks, and makes exactly one `POST` create call while holding them, with a compensating `DELETE` and a marker-based clean-up (`agenda` carries `Polymath TMD ZL-{pk:04d}`) if anything fails afterwards (task 006 D6). The IT queue (`templates/zoom/queue.html`, `detail.html`) and the Zoom accounts screens (`zoom:accounts`, `zoom:account_add`, `zoom:account_edit`, task 008) are both in-app; `apps/zoom/admin.py` is gone. The Zoom timetable (`zoom:timetable`, `zoom:timetable_day`, task 009) is a read-only month/day view of booked and waiting classes; its month arithmetic, week layout and grouping are pure functions with no database access in `apps/zoom/timetable.py`, so the views stay thin and each page runs a fixed number of queries. `HostAccount.stop_booking_errors()` locks both the account and its booked `Occurrence`/`LinkRequest` rows (`select_for_update(of=("self", "link_request"))`) before checking them, so the check stays correct even if the isolation level ever becomes REPEATABLE READ (task 008, review round 1). Joining `IT desk` is done on `accounts`' Staff and access page (task 010), same as any other role. `services.cancel()` (task 011) follows `approve()`'s lock shape: it locks `LinkRequest`, then `HostAccount`, then its `Occurrence` rows, writes every change, makes exactly one Zoom `DELETE` inside that same transaction, then commits — Zoom first would leave a window where task 013's future webhook could cancel the booking under the wrong source and double-email it. The approval email no longer carries the host key; instead it carries a signed, expiring **start link** (`services.start_token()` / `zoom:start`, no sign-in) that redirects to a fresh Zoom `start_url` only inside each class's 30-minute-before-to-end window, never stores or logs that URL, and is deliberately kept out of the Django admin equivalent everywhere (`no-store`, `no-referrer` on every response). The host key itself is never emailed any more, but stays as an IT-only, recorded fallback: `zoom.change_hostaccount` holders can read it back once, on its own page, through `services.reveal_host_key()`, and every reveal writes a `HostKeyReveal` row (task 011 D11).
...
- **A new public URL that carries a secret in its path must be added to `config/log_filters.py`'s redaction rule** (task 011), the same way `/zoom/start/<token>/` is — otherwise it's logged in full by gunicorn's access log and Django's own request/security loggers, in plain text, on every request.
